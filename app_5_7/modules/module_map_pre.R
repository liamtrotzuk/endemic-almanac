...
map_pre <- function(input,output,session) {
  ...
  MAP_initial_final <- leaflet(DF_MapLeaflet_B) %>%  
    ...
    addMarkers(-74.217933, 43.299428,icon = icons(iconUrl = "images/map-marker-3.svg",
                                                  ...
                                                  iconAnchorX = 15,
                                                  iconAnchorY = 29)) %>% 
    setView(-74.217933, 43.299428, zoom = 4)
  
  REACTIVE_MAP_Pre <- reactive(renderLeaflet(MAP_initial_final))
